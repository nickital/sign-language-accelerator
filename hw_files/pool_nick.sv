...
module pool (
  ...
  enum {IDLE, SETUP, READ_ROWS, WRITE, DONE} next_state, state; //state machine 
  ...
  logic [ARR_IDX_W-1:0] pool_out_row_idx_hw;
  logic [ARR_IDX_W-1:0] pool_out_row_idx_hw_ps;
  ...
  assign pool_arr_in_dim     = slrx_regs_intrf.host_regs[ARR_IN_DIM_RI];
  ...
  always_comb begin
   ...
   mem_intf_write.mem_data       = calc_pool_win(pool_in_buf) ;
   mem_intf_write.mem_start_addr = pool_rslt_out_addr ;
   
   pool_rslt_out_addr_ps = pool_arr_out_addr + (pool_out_row_idx_hw*pool_arr_out_dim);
   ...
   pool_out_row_idx_hw_ps = pool_out_row_idx_hw;
   ...
   case (state) // State Machine case
      ...
      IDLE: if (pool_start) begin
       pool_done = 0;
       if      (slrx_cmd==POOL_SETUP)  next_state = SETUP;
       else if (slrx_cmd==POOL_CALC) begin
         next_state = READ_ROWS;
         pool_out_row_idx_hw_ps = 0;
         arr_in_row_addr_ps = pool_arr_in_addr;
       end
       load_next_row_idx = 0;       
      end
      ...
      READ_ROWS: begin
        ...
        if (mem_intf_read.mem_valid) begin
          ...
          if (load_row_idx==1) begin // Loading always two rows
             // The max-pool row is purely combinational, so do not spend an
             // extra FSM cycle in a separate CALC state. WRITE calculates
             // directly from the registered two-row input buffer.
             next_state = WRITE ;  
             mem_intf_read.mem_req = 0;
          end else begin
      ...
      WRITE: begin
        ...
        if (mem_intf_write.mem_ack) begin
          mem_intf_write.mem_req = 0;
          if (pool_out_row_idx_hw < pool_arr_out_dim - 1) begin
            pool_out_row_idx_hw_ps = pool_out_row_idx_hw + 1;
            arr_in_row_addr_ps = pool_arr_in_addr +
                                 ((pool_out_row_idx_hw + 1) * 2 * pool_arr_in_dim);
            load_next_row_idx = 0;
            next_state = READ_ROWS;
          end else begin
            next_state = DONE;
 ...
 // Sequential
  always @(posedge clk or negedge rst_n) begin
    ...
    if(!rst_n) begin  
      ...
      pool_out_row_idx_hw    <= pool_out_row_idx_hw_ps;
    end
  end
