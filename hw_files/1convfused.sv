...
module conv (
  ...
  // Fused conv+pool support. For an even convolution row, store horizontal
  // pair maxima. For the following odd row, combine with those saved values
  // and write the pooled row directly, skipping the intermediate conv map.
  logic conv_pool_mode;
  logic [ARR_IDX_W:0] conv_pool_out_dim;
  logic [ARR_IDX_W:0] conv_pool_wr_byte_count;
  logic [XMEM_ADDR_WIDTH-1:0] conv_pool_rslt_out_addr;
  logic [DIM_MAX_SIZE/2-1:0][7:0] conv_pool_top_buf, conv_pool_top_buf_ps;
  logic [CONV_PAR_WIDTH/2-1:0][7:0] conv_pool_out_val_par;
  ...
  assign conv_active = (slrx_cmd == CONV_SETUP) || (slrx_cmd == CONV_WINDOW) || (slrx_cmd == CONV_POOL_WINDOW);
  ...
  assign conv_pool_mode = (slrx_cmd == CONV_POOL_WINDOW);
  ...
  assign conv_pool_out_dim = conv_arr_out_dim >> 1;
  ...
  assign conv_pool_rslt_out_addr = conv_arr_out_addr +
                                   ((conv_out_row_idx >> 1) * conv_pool_out_dim) +
                                   (conv_out_col_idx >> 1);
  ...
  assign conv_pool_wr_byte_count = conv_wr_byte_count >> 1;
  ...
  always_comb begin
    ...
    conv_pool_top_buf_ps = conv_pool_top_buf;
    ...
    case (state)
      ...
      IDLE: 
        if (conv_start) begin
          ...
          else if ((slrx_cmd == CONV_WINDOW) || (slrx_cmd == CONV_POOL_WINDOW)) begin
      ...
      //=======================================================================
      WRITE: begin
        // In fused mode, even convolution rows only fill conv_pool_top_buf.
        // No intermediate conv row is written. Odd rows write pooled bytes.
        if (conv_pool_mode && (conv_out_row_idx[0] == 1'b0)) begin
  ...
  always_comb begin
    for (int pp = 0; pp < (CONV_PAR_WIDTH/2); pp++) begin
      logic [7:0] max_top;
      logic [7:0] max_bot;
      max_top = conv_pool_top_buf[(conv_out_col_idx >> 1) + pp];
      max_bot = (conv_out_val_par_ps[2*pp] > conv_out_val_par_ps[2*pp+1]) ?
                conv_out_val_par_ps[2*pp] : conv_out_val_par_ps[2*pp+1];
      conv_pool_out_val_par[pp] = (max_top > max_bot) ? max_top : max_bot;
    end
  end
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin  
      ...
      conv_pool_top_buf <= conv_pool_top_buf_ps;
      ...
      if (state == CALC) begin
        ...
        if (conv_pool_mode) begin
          if (conv_out_row_idx[0] == 1'b0) begin
            // Save horizontal max for the top row of each 2x2 pool window.
            for (int pp = 0; pp < (CONV_PAR_WIDTH/2); pp++) begin
              conv_pool_top_buf[(conv_out_col_idx >> 1) + pp] <=
                (conv_out_val_par_ps[2*pp] > conv_out_val_par_ps[2*pp+1]) ?
                 conv_out_val_par_ps[2*pp] : conv_out_val_par_ps[2*pp+1];
            end
            conv_wr_byte_count_lat <= 0;
            conv_wr_addr_lat       <= conv_pool_rslt_out_addr;
            conv_wr_data_lat       <= '0;
          end else begin
            // Bottom row: combine bottom horizontal max with saved top max and write pooled row.
            conv_wr_byte_count_lat <= conv_pool_wr_byte_count;
            conv_wr_addr_lat       <= conv_pool_rslt_out_addr;
            for (int pp = 0; pp < (CONV_PAR_WIDTH/2); pp++)
              conv_wr_data_lat[pp] <= conv_pool_out_val_par[pp];
            for (int b = (CONV_PAR_WIDTH/2); b < DIM_MAX_SIZE; b++)
