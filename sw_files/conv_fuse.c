...
void conv_layer_xlr(){  
    ...
    // Trigger the hardware to execute the ENTIRE layer
    HOST_REG(XLR_START_RI) = CONV_WINDOW ; 
  
    while (!HOST_REG(XLR_DONE_RI)) {
       //printf("Conv layer Polling ...\n"); 
    }
...
// Fused hardware path: convolution followed immediately by 2x2 max-pool.
// conv_pool_arr_out points to the pooled output buffer, not the intermediate conv buffer.
void conv_pool_2x2_xlr(uint8_t* conv_pool_arr_out,
    ...
    HOST_REG(CONV_BIAS_VAL_RI)   = kernel_b;
    HOST_REG(ARR_IN_ADDR_RI)     = (unsigned int)conv_arr_in;
    HOST_REG(ARR_OUT_ADDR_RI)    = (unsigned int)conv_pool_arr_out;
    HOST_REG(ARR_IN_DIM_RI)      = arr_in_dim;
    ...
    HOST_REG(XLR_START_RI) = CONV_POOL_WINDOW;
    while (!HOST_REG(XLR_DONE_RI)) {
    }
...
// Wrapper used by infer(). With HW enabled it uses the fused conv+pool command.
// Otherwise it falls back to the original separate software/HW calls.
void conv_pool_2x2(uint8_t* pool_arr_out,
    ...
    #if defined(CONV_XON) && defined(POOL_XON)
    conv_pool_2x2_xlr(pool_arr_out, conv_arr_in, arr_in_dim, kernel_w, kernel_b);
    #else
    conv(conv_tmp_arr_out, conv_arr_in, arr_in_dim, kernel_w, kernel_b);
    pool_max_2x2(pool_arr_out, conv_tmp_arr_out, arr_in_dim - CONV_KERNEL_DIM + 1);
    #endif
}
...
void conv(uint8_t* conv_arr_out,                               
    ...
    #ifdef CONV_XON
    // Hardware Setup and Execution: Just two calls!
    conv_xlr_setup(conv_arr_out, conv_arr_in, arr_in_dim, kernel_w, kernel_b);       
    conv_layer_xlr(); 
    #else
    // Pure software execution loops remain unchanged
